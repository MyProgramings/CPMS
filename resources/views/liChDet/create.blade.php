<x-app-layout>
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-gray-700 ltr:text-left rtl:text-right dark:text-gray-300" id="h3_Title">
                        {{ __('app.create') }} {{ __('app.Check_Up_Details') }}</h3>
                </div>
            </div>
        </div>
    </div>
    <div class="flex border-t flex-wrap items-center">
        <div class="relative flex-row flex-1 w-full max-w-full px-4 py-4">
            <form action="{{ Route('li_ch_dets.store') }}" method="POST">
                @csrf
                <div class="relative grid grid-cols-6 gap-6 mt-2">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        <x-jet-label for="list_checkup_id" value="{{ __('app.Category') }}" />
                        <x-select class="mt-1" name="list_checkup_id" :value="old('list_checkup_id')">
                            <option value="{{ __(old('list_checkup_id')) }}" readonly="true" hidden="true">
                                {{ __(old('list_checkup_id')) }}</option>
